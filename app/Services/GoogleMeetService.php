...
class GoogleMeetService
{
     ...
     * @param object|null $course Thông tin sản phẩm (nếu null sẽ tạo một tên mặc định)
     * @param object|null $teacher Thông tin giáo viên (nếu null sẽ tạo một tên mặc định)
     * @return string|null URL của cuộc họp Google Meet hoặc null nếu có lỗi
     */
    public function createMeetLinkForTimetable(Timetable $timetable, $course = null, $teacher = null): ?string
    {
        try {
            // Kiểm tra xem đã xác thực chưa
            if (!$this->client->getAccessToken()) {
            ...
            $courseName = $course ? ($course->name_p ?? $course->course_name ?? 'Lớp học') : 'Lớp học';
            $teacherName = $teacher ? ($teacher->fullname_t ?? $teacher->name_t ?? 'Giáo viên') : 'Giáo viên';

            $summary = "Lớp học: {$courseName}";
            ...
            return $this->createMeetLink($summary, $startTime, $endTime, $description);
        } catch (\Exception $e) {
     ...
     * 
     * @param string $dayName Tên ngày trong tuần (ví dụ: 'Monday', 'Tuesday', 'Thứ 2', 'Thứ 3',...)
    ...
    private function getNextDayOfWeek(string $dayName): string
    {
        ...
        if ($dayNum === null) {
            // Log lỗi để debug
            Log::error('Tên ngày không hợp lệ: ' . $dayName);
            throw new \Exception('Tên ngày không hợp lệ: ' . $dayName);
        }
